...
# --- Layout: 2x2 grid, then full-width competitor panel below ---

# Row 1: Meeting Briefing | Salesforce Data
col1, col2 = st.columns(2)
with col1:
    st.subheader("Meeting Briefing")
    pdf_path = "Nest Corporation – Emerging Markets Equity Mandate Briefing.pdf"
    try:
        import io
        import base64
        ...
        if doc.page_count == 0:
            ...
            for page_num in range(doc.page_count):
                ...
                images_html += f'<img src="data:image/png;base64,{img_b64}" style="width:100%;margin-bottom:16px;" alt="Page {page_num+1}"/>'
            scrollable_html = f'''
            <div style="max-height:500px;overflow:auto;padding:8px;border:1px solid #ddd;background:#fafafa;">
                {images_html}
            </div>
            '''
...
with col3:
    ...
    pdf_path = "Engagement Report_ Nest Corporation – Emerging Markets Equity Mandate (2025 YTD) (1).pdf"
    try:
        import io
        import base64
        from PIL import Image
...
with col4:
    ...
    # Scrollable window for the table
...
# Row 3: Full-width Competitor Data panel
with st.container():
    st.markdown("---")
